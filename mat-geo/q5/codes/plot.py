...
a = float(input("Enter the value of 'a' for the parabola equation y^2 = ax: "))
r = float(input("Enter the radius 'r' for the circle equation x^2 + y^2 = r^2: "))
...
lib.solve_intersection(x_intersections.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
                       ...
                       ctypes.byref(num_intersections), a, r)

# Plotting
plt.figure()

# Plot the circle
plt.plot(x_circle, y_circle, label=f'Circle (r = {r})', color='blue')

# Plot the parabola (both positive and negative y-values)
plt.plot(x_parabola, y_parabola, label=f'Parabola (y^2 = {a}x)', color='orange')
...
if num_intersections.value > 0:
    ...
    for i in range(num_intersections.value):
        label = r"$(x = {:.0f}a, y = \pm2\sqrt{{3}}a)$".format(x_intersections[i])
...
inside_circle = (x_parabola)**2 + (y_parabola)**2 <= r**2
...
plt.xlabel("x")
plt.ylabel("y")
plt.title("Parabola and Circle Intersection with Labeled Points (√ symbol)")
plt.grid(True)
plt.legend()
